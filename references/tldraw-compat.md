...
1. Generate smoke file:
   `node scripts/gen-tldr.mjs --smoke`
...
The `schema.sequences` object in `gen-tldr.mjs` is pinned to a specific tldraw version.
If the tldraw VS Code extension updates its internal schema and generated files start
showing a blank canvas again, the `SCHEMA` constant in `gen-tldr.mjs` must be updated.

To extract current schema from a working `.tldr` file:

1. Create a new diagram manually in tldraw (VS Code).
2. Save it and open the `.tldr` file as JSON.
3. Copy the `schema` object into the `SCHEMA` constant in `gen-tldr.mjs`.
